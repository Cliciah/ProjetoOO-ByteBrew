...
public class ClienteStandard extends ClienteCadastrado {
    ...
    public void pagarXP(double valorCompra) {
        ...
        if (this.saldoAcumuladoXP < custoPontos) {
            throw new PontosInsuficienteException(String.format(
                "Saldo insuficiente: necessario %d XP para cobrir metade da compra, voce tem disponivel: %d XP",
                custoPontos, saldoAcumuladoXP));
        }

        this.saldoAcumuladoXP -= custoPontos;
        System.out.printf(
            "Pedido de R$ %.2f pago em dinheiro + %d XP (equivalente a R$ %.2f). Saldo atual: %d XP.%n",
            valorDinheiro, custoPontos, valorCobertoXP, saldoAcumuladoXP);
    }
}